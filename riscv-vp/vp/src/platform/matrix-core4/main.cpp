...
public:
	...
	addr_t dma2_start_addr = 0x70002000;
	addr_t dma2_end_addr = 0x70003000;
	addr_t dma3_start_addr = 0x70004000;
	addr_t dma3_end_addr = 0x70005000;
	addr_t dma4_start_addr = 0x70006000;
	addr_t dma4_end_addr = 0x70007000;
	...
	addr_t matrixMul_start_addr = 0x73000000;
 	addr_t matrixMul_size = 0x01000000;
 	addr_t matrixMul_end_addr = matrixMul_start_addr + matrixMul_size - 1;
	 
	addr_t matrixMul2_start_addr = 0x74000000;
 	addr_t matrixMul2_size = 0x01000000;
 	addr_t matrixMul2_end_addr = matrixMul2_start_addr + matrixMul2_size - 1;
	
	addr_t matrixMul3_start_addr = 0x75000000;
 	addr_t matrixMul3_size = 0x01000000;
 	addr_t matrixMul3_end_addr = matrixMul3_start_addr + matrixMul3_size - 1;

	addr_t matrixMul4_start_addr = 0x76000000;
 	addr_t matrixMul4_size = 0x01000000;
 	addr_t matrixMul4_end_addr = matrixMul4_start_addr + matrixMul4_size - 1;
	...
	void parse(int argc, char **argv) override {
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > matrixMul_end_addr) &&
...
int sc_main(int argc, char **argv) {
	...
	MatrixMul matrix_mul("matrix_mul");
	MatrixMul matrix_mul2("matrix_mul2");
	MatrixMul matrix_mul3("matrix_mul3");
	MatrixMul matrix_mul4("matrix_mul4");
	MemoryDMI dmi = MemoryDMI::create_start_size_mapping(mem.data, opt.mem_start_addr, mem.size);
	InstrMemoryProxy instr_mem(dmi, core0);
	InstrMemoryProxy instr_mem2(dmi, core1);
	InstrMemoryProxy instr_mem3(dmi, core2);
	InstrMemoryProxy instr_mem4(dmi, core3);
	...
	core2.init(&core2_mem_if, &core2_mem_if, &clint, loader.get_entrypoint(), opt.mem_end_addr - 65532);
	core3.init(&core3_mem_if, &core3_mem_if, &clint, loader.get_entrypoint(), opt.mem_end_addr - 98297);
	...
	bus.ports[6] = new PortMapping(opt.dma2_start_addr, opt.dma2_end_addr);
	bus.ports[7] = new PortMapping(opt.dma3_start_addr, opt.dma3_end_addr);
	bus.ports[8] = new PortMapping(opt.dma4_start_addr, opt.dma4_end_addr);
	bus.ports[9] = new PortMapping(opt.sensor2_start_addr, opt.sensor2_end_addr);
	bus.ports[10] = new PortMapping(opt.mram_start_addr, opt.mram_end_addr);
	bus.ports[11] = new PortMapping(opt.flash_start_addr, opt.flash_end_addr);
	bus.ports[12] = new PortMapping(opt.ethernet_start_addr, opt.ethernet_end_addr);
	bus.ports[13] = new PortMapping(opt.display_start_addr, opt.display_end_addr);
	bus.ports[14] = new PortMapping(opt.sys_start_addr, opt.sys_end_addr);
	bus.ports[15] = new PortMapping(opt.matrixMul_start_addr, opt.matrixMul_end_addr);
	bus.ports[16] = new PortMapping(opt.matrixMul2_start_addr, opt.matrixMul2_end_addr);
	bus.ports[17] = new PortMapping(opt.matrixMul3_start_addr, opt.matrixMul3_end_addr);
	bus.ports[18] = new PortMapping(opt.matrixMul4_start_addr, opt.matrixMul4_end_addr);
	...
	PeripheralWriteConnector dma_connector2("SimpleDMA2-Connector");  // to respect ISS bus locking
	dma_connector2.isock.bind(bus.tsocks[5]);
	dma2.isock.bind(dma_connector2.tsock);
	dma_connector2.bus_lock = bus_lock;

	PeripheralWriteConnector dma_connector3("SimpleDMA3-Connector");  // to respect ISS bus locking
	dma_connector3.isock.bind(bus.tsocks[6]);
	dma3.isock.bind(dma_connector3.tsock);
	dma_connector3.bus_lock = bus_lock;

	PeripheralWriteConnector dma_connector4("SimpleDMA4-Connector");  // to respect ISS bus locking
	dma_connector4.isock.bind(bus.tsocks[7]);
	dma4.isock.bind(dma_connector4.tsock);
	dma_connector4.bus_lock = bus_lock;
	...
	bus.isocks[6].bind(dma2.tsock);
	bus.isocks[7].bind(dma3.tsock);
	bus.isocks[8].bind(dma4.tsock);
	bus.isocks[9].bind(sensor2.tsock);
	bus.isocks[10].bind(mram.tsock);
	bus.isocks[11].bind(flashController.tsock);
	bus.isocks[12].bind(ethernet.tsock);
	bus.isocks[13].bind(display.tsock);
	bus.isocks[14].bind(sys.tsock);
	bus.isocks[15].bind(matrix_mul.tsock);
	bus.isocks[16].bind(matrix_mul2.tsock);
	bus.isocks[17].bind(matrix_mul3.tsock);
	bus.isocks[18].bind(matrix_mul4.tsock);
