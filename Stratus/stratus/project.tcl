...
define_hls_module Matrix_mul ../Matrix_mul.cpp

### 4. Define your HLS configuration (arbitrary names, BASIC and DPA in this example). 
define_hls_config Matrix_mul BASIC
define_hls_config Matrix_mul DPA       --dpopt_auto=op,expr
define_hls_config Matrix_mul PIPELINE  -DII=8
define_hls_config	Matrix_mul UNROLL    --dpopt_auto=op,expr

set IMAGE_DIR           ".."
set IN_FILE_NAME        "${IMAGE_DIR}/in.txt"
...
define_sim_config V_BASIC "Matrix_mul RTL_V BASIC" -argv "$IN_FILE_NAME $OUT_FILE_NAME"
### 5.3 The Verilog simulation for HLS config "DPA". 
define_sim_config V_DPA "Matrix_mul RTL_V DPA" -argv "$IN_FILE_NAME $OUT_FILE_NAME"

define_sim_config V_PIPELINE "Matrix_mul RTL_V PIPELINE" -argv "$IN_FILE_NAME $OUT_FILE_NAME"

define_sim_config V_UNROLL "Matrix_mul RTL_V UNROLL" -argv "$IN_FILE_NAME $OUT_FILE_NAME"
